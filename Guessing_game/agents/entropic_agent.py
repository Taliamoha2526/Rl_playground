...
class EntropicAgent(BaseAgent):
    ...
    def __init__(
        self, search_space, override_probability=0.1, max_steps=20
    ):
    ...
    def choose_feature_entropy(self):
        feature_scores = {}
        last_asked_feature = (
            list(self.history)[-1][0] if len(self.history) > 0 else None
        )
    ...
    def take_turn(self):
        ...
        # Random unasked question exploration override
        if random.random() < self.override_probability:
            unasked_features = [
                col
                for col in self.search_space.columns
                if col not in self.asked_features
            ]
            if unasked_features:
                r_feat = random.choice(unasked_features)
                r_val = random.choice(
                    list(self.search_space[r_feat].dropna().unique())
                )
