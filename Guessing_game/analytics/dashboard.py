...
def plot_comparative_dashboard(df_results):
    ...
    fig.suptitle(
        "Multi-Agent Performance Comparison Dashboard",
        fontsize=15,
        fontweight="bold",
    )
    ...
    for i, v in enumerate(win_rates.values):
        axes[0, 0].text(
            i, v + 2, f"{v:.1f}%", ha="center", fontweight="bold"
        )

    # Average steps in a winning game
    wins_only = df_results[df_results["won"]]
    step_data = [
        wins_only[wins_only["agent"] == agent]["steps"] for agent in agents
    ]
    ...
    for idx, agent in enumerate(agents):
        ...
        padded_histories = np.array(
            [
                ...
                for h in agent_df["space_history"]
            ]
        )
